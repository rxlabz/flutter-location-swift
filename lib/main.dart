...
class _HelloServicesState extends State<HelloServices> {
  ...
  Future<Null> _getLocation() async {
    print('_HelloServicesState._getLocation...');
    final Map<String, String> message = <String, String>{'provider': 'gps'};
    ...
    setState(() {
      print('update... $reply ');
      if( reply != null && reply.containsKey('latitude')){
        _latitude = reply['latitude'].toDouble();
        _longitude = reply['longitude'].toDouble();
      } else {
        print('_HelloServicesState._getLocation... ERROR !!!');
        _latitude = 0.0;
        _longitude = 0.0;
      }
    });
